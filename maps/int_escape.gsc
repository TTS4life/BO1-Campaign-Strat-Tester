...
event5_movie_control()
{
	...
	{
		start3dcinematic("int_reznov_room", true , true);
	}
...
event6_limo_warps()
{
	...
	level.fullscreen_cin_hud.alpha = 0;
	wait 1;
	thread play_movie("int_kennedy_pentagon_flash", false, true, "jfk_movie_play", undefined, "jfk_movie_done", 0.1);
	start_movie_scene();
	...
	level thread maps\int_escape_amb::event_heart_beat("none", 0, 1);
	int_esc_play_movie("int_flaming_limo");
...
{
	thread int_esc_play_movie("int_drag_krav_stein_flash");
	level.fullscreen_cin_hud.alpha = 0;
	wait 0.15;
	level.fullscreen_cin_hud.alpha = 1;
	wait 1;
	...
	while(1)
	{
		...
		if (last_movement > 20)
		{
			if (cointoss() )
			{
				...
				int_esc_play_movie("int_reznov_huecity_flash");
			}
			else
			{
				//playsoundatposition ("evt_steiner_wmd_front", (0,0,0));
				int_esc_play_movie("int_steiner_wmd_flash");
